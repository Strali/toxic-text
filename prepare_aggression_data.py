...
'''
toxicity_labels = \
    (toxicity_annotations.groupby('rev_id', as_index=False)['toxicity'].mean() > 0.5) & \
    (toxicity_annotations.groupby('rev_id', as_index=False)['toxicity'].mean() <= 0.8)

severe_toxicity_labels = toxicity_annotations.groupby(
    'rev_id', as_index=False)['toxicity'].mean() > 0.8
'''
...
'''
only_severe_toxic_comments = pd.DataFrame(
    toxicity_comments[toxicity_comments['severe_toxicity']]['comment_text'])
'''
...
for cat in class_list:
    if cat == 'severe_toxic':
        only_severe_toxic_comments[cat] = 1
    else:
        only_severe_toxic_comments[cat] = 0
'''
only_toxic_comments.to_csv('./data/extra_toxic_comments.csv',
                           ...
                           encoding='utf-8')
'''
only_severe_toxic_comments.to_csv('./data/extra_severe_toxic_comments.csv',
                                  sep=',',
                                  index=False,
                                  encoding='utf-8')
'''
